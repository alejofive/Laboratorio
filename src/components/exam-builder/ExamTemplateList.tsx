      ...
      <div className='mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between'>
        <div>
          ...
          <p className='mt-1 max-w-2xl text-sm text-secondary'>
            Crea y administra las estructuras dinamicas que luego se usan para cargar resultados de
            pacientes.
          </p>
        </div>
        <Link
          href='/dashboard/exam-templates/create'
          className='inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-brand-primary bg-brand-primary px-4 py-2 text-base font-medium text-white transition-colors hover:bg-brand-primary/90 hover:text-white'
        >
          <FilePlus2 size={18} />
          Crear nuevo examen
        </Link>
      ...
      <div className='rounded-3xl border border-border-default bg-surface'>
        ...
        {!isLoading && templates.length === 0 ? (
          <div className='p-10 text-center'>
            ...
            <p className='mt-2 text-sm text-secondary'>
              Crea la primera plantilla para dejar de depender de formularios fijos.
            </p>
          </div>
        ) : null}
        ...
        {!isLoading && templates.length > 0 && filteredTemplates.length === 0 ? (
          <div className='p-10 text-center'>
            ...
            <p className='mt-2 text-sm text-secondary'>
              Prueba con otro nombre, categoria o campo.
            </p>
          </div>
        ) : null}
              ...
              >
                <div>
                  <div className='flex flex-wrap items-center gap-2'>
                    ...
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-bold ${template.is_active ? 'bg-success-soft text-success' : 'bg-surface-muted text-secondary'}`}
                    >
