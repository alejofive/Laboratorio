...
export function FieldLabel({ children, className = '', ...props }: FieldLabelProps) {
  return (
    <label className={mergeClassName('mb-1 block text-sm font-medium text-tertiary', className)} {...props}>
      {children}
    </label>
  )
}

export const TextInput = forwardRef<HTMLInputElement, InputHTMLAttributes<HTMLInputElement> & BaseFieldProps>(
...
export const SelectInput = forwardRef<HTMLSelectElement, SelectHTMLAttributes<HTMLSelectElement> & BaseFieldProps>(
...
export const TextareaInput = forwardRef<HTMLTextAreaElement, TextareaHTMLAttributes<HTMLTextAreaElement> & BaseFieldProps>(
  function TextareaInput({ className = '', error, ...props }, ref) {
    return (
      <textarea
        ref={ref}
        className={mergeClassName(fieldBaseClass, 'min-h-24 resize-y', getBorderClass(error), className)}
        {...props}
      />
    )
  },
...
export const CheckboxInput = forwardRef<HTMLInputElement, CheckboxInputProps>(
  function CheckboxInput({ label, className = '', ...props }, ref) {
    return (
      <label className={mergeClassName('flex cursor-pointer items-center gap-2 text-sm font-medium text-tertiary', className)}>
        <input
          ref={ref}
          type="checkbox"
